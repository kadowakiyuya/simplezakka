...
class ProductControllerTest {
    ...
    private ProductListItem productListItem3;
    ...
    void setUp() {
        ...
        productDetailWithNulls = new ProductDetail(3, "詳細商品3", 300, null,
                                                   null, null,5, null);

        // --- Serviceメソッドのデフォルトモック設定 (lenient) ---
        // デフォルトではfindAllProductsは2つのアイテムを返す
        lenient().when(productService.getFilteredAndSortedProducts(anyString(), anyString(), anyString()))
    ...
    class getFilteredAndSortedProductsTests {

        @Test
        @DisplayName("商品が存在する場合、指定の引数で商品リスト(ProductListItem)を200 OKで返す")
        void getgetFilteredAndSortedProducts_WhenProductsExist_ShouldReturnProductList() throws Exception {
            ...
            verify(productService, times(1)).getFilteredAndSortedProducts("","","new");
            verifyNoMoreInteractions(productService);
        }

        @Test
        @DisplayName("商品が存在しない場合、空のリストを200 OKで返す")
        void getFilteredAndSortedProducts_WhenServiceThrowsException_ShouldReturnEmptyList() throws Exception {
            // Arrange
             String  keyword = "";
            String  category = "";
            String  sort = "new";
            when(productService.getFilteredAndSortedProducts(eq(keyword), eq(category), eq(sort))).thenReturn(Collections.emptyList()); // 空リストを返すように設定
            ...
            verify(productService, times(1)).getFilteredAndSortedProducts(eq(keyword), eq(category), eq(sort));
            verifyNoMoreInteractions(productService);
        }

        @Test
        @DisplayName("ProductServiceが例外をスローした場合、500 Internal Server Errorを返す")
        void getFilteredAndSortedProducts_WhenServiceThrowsException_ShouldReturnInternalServerError() throws Exception {
            // Arrange
             String  keyword = "";
            String  category = "";
            String  sort = "new";
            when(productService.getFilteredAndSortedProducts(eq(keyword), eq(category), eq(sort))).thenThrow(new RuntimeException("サービスエラー"));
